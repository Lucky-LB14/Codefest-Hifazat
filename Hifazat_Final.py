...
def run_face_mp(image):
    ...
    if results.face_landmarks :
        ...
        if results.right_hand_landmarks:
            try:
                keypoint10_x = results.pose_landmarks.landmark[mp_holistic.PoseLandmark.RIGHT_INDEX].x
                keypoint10_y = results.pose_landmarks.landmark[mp_holistic.PoseLandmark.RIGHT_INDEX].y
                g1 = math.sqrt(pow((keypoint10_x- x), 2) + pow((keypoint10_y - y), 2))
                #print ("right hand",g1)
                handy = g1
                
                keypoint100_x = results.pose_landmarks.landmark[mp_holistic.PoseLandmark.LEFT_INDEX].x
                keypoint100_y = results.pose_landmarks.landmark[mp_holistic.PoseLandmark.LEFT_INDEX].y
                g2 = math.sqrt(pow((keypoint10_x- x2), 2) + pow((keypoint10_y - y2), 2))
                #print("left hand",g2)
    ...
    while cap.isOpened():
        ...
        if frame_before_run >= 15 and len(input_data) == 20:
            ...
            if ear_main < -1.5:
                ...
                print(label)
                cv2.putText(image, "lethargic", (int(0.02 * image.shape[1]), int(0.2 * image.shape[0])),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255,0,0), 2)
                ...
                label="looking_Left"
                print(label)
                cv2.putText(image, "looking left", (int(0.02 * image.shape[1]), int(0.2 * image.shape[0])),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255,0,0), 2)
                
            elif distance1 > distance1cal+50:
                label="looking_right"
                print(label)
                cv2.putText(image, "looking right", (int(0.02 * image.shape[1]), int(0.2 * image.shape[0])),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255,0,0), 2)
                
            elif handy <= 1100 and handy >850 :

                label = "Talking on Phone"
                handy = 0
                cv2.putText(image, "Talking on phone", (int(0.02 * image.shape[1]), int(0.2 * image.shape[0])),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255,0,0), 2)
                ser = serial.Serial('/dev/ttyACM0', 115200, timeout=1)
                ser.reset_input_buffer()
                ser.write(b"tp")
                ...
                label = "Neutral"
                print(label)
                cv2.putText(image, "Neutral", (int(0.02 * image.shape[1]), int(0.2 * image.shape[0])),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255,0,0), 2)
